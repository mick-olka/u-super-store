...
import type { I_Product, I_ProductsListRes } from "@/shared/models";

import { revalidation } from "./data";

export async function getProducts(search?: string): Promise<I_ProductsListRes> {
	const link = search
		? `${globalConfig.apiUrl}/products?regex=${search}`
		: `${globalConfig.apiUrl}/products`;
...
export async function getProductById(id: string): Promise<I_Product> {
	const res = await fetch(
		`${globalConfig.apiUrl}/products/${id}`,
		revalidation,
	);
	if (!res.ok) {
		throw new Error("Failed to fetch data");
