...
export default async function Product({ params }: Props) {
	...
	const getBreadCrumps = () => {
		...
		if (category) {
			return [
				{
					name: category.name ? category.name[lang] : category.url_name,
					link: localeUrl(
						`${E_AppRoutes.collection}/${category.url_name}`,
						lang,
					),
				},
				{ name: product.name[lang], link: product.url_name },
			];
		}
		return [{ name: product.name[lang], link: product.url_name }];
	};
	return (
		<div className="p-4">
			<div className="py-6">
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
					<div className="flex flex-col items-center xl:items-start xl:flex-row gap-4 -mx-4">
						<Gallery
							...
							defaultPhoto={`${globalConfig.apiUrl}/upload/${product.thumbnail}`}
						/>
						<div className="md:flex-1 px-4">
							...
							<div className="flex py-4 space-x-4">
								<AddToCartPane product={product} lang={lang} />
							</div>
					...
					<FeaturesList list={product.features} lang={lang} />
					{product.related_products.length ? (
						<div className="mt-6">
							<h2 className="text-lg font-bold">
								{dictionary.product.related_products}
							</h2>
							<ProductsGrid products={product.related_products} lang={lang} />
						</div>
					) : null}
					{product.similar_products.length ? (
						<div className="mt-6">
							<h2>{dictionary.product.similar_products}</h2>
							<ProductsGrid products={product.similar_products} lang={lang} />
						</div>
					) : null}
				</div>
			</div>
