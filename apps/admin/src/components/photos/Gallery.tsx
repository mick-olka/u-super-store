...
import type { ButtonClickEvent } from "src/models";
	...
	return (
		<Box sx={{ display: "flex", overflow: "auto", width: "100%" }}>
			{props.path_arr.map((path) => (
				...
				>
					<PS.PhotoOverlay
						onClick={() => onPhotoOpenClick(`${globalConfig.photosUrl}${path}`)}
					>
						{props.editMode && (
							<S.RoundButton
			...
			{props.newFiles.map((f) => (
				...
				>
					<PS.PhotoOverlay
						onClick={() => onPhotoOpenClick(URL.createObjectURL(f))}
					>
						{props.editMode && (
							<S.RoundButton
