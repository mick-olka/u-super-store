...
import { E_Languages, type I_ProductFeatures } from "src/models";
...
export const FeaturesManager = ({ features, onChange }: I_Props) => {
	...
	const addFeature = () => {
		onChange({
			...features,
			[featuresLan]: [...features[featuresLan], { key: "", value: "" }],
		});
	};
	...
	return (
		<S.FeaturesManagerWrapper>
			...
			{features[featuresLan].map((f, i) => (
				<Box key={`f${f.value}-${i}`} sx={{ display: "flex" }}>
					<CS.TextFieldStyled
						value={f.key}
						size="small"
