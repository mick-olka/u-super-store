...
import type { I_Product } from "src/models";
...
export const ChooseProducts = ({
	...
	const { products, count, isLoading, isError, refetch, limit } = useProducts({
		page,
		regex,
	});
	const products_filtered = useMemo(() => {
		...
		if (products && collectionId)
			res = res.map((p) => ({
				...p,
				disabled: p.collections
					? !!p.collections.find((c) => c._id === collectionId)
					: false,
			}));
		if (products && exclude)
			res = res.filter((p) => ({ ...p, disabled: !exclude.includes(p._id) }));
		return res;
