...
import {
	E_Queries,
	type I_CollectionDto,
	type I_CollectionItemsDto,
} from "src/models";
...
export const useCreateCollection = () => {
	...
	const { mutateAsync, data, isLoading, isError } = useMutation(
		(form_data: I_CollectionDto) =>
			toasterPending(CollectionService.create(form_data)),
...
export const useUpdateCollection = (validation_id?: string) => {
	...
	const { mutateAsync, data, isLoading, isError } = useMutation(
		({ id, form_data }: { id: string; form_data: I_CollectionDto }) => {
			if (form_data.items) form_data.items = undefined; // for items use updateCollectionItems (PUT)
