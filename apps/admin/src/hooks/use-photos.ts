...
import { E_Queries, type I_PhotosDto } from "src/models";
...
export const useCreatePhotosGroup = (
	product_id: string,
	product_url: string,
) => {
	...
	const { mutateAsync, data, isLoading, isError } = useMutation(
		(data: I_PhotosDto) =>
			toasterPending(PhotosService.create(product_id, data)),
		{
...
export const useUpdatePhotosGroup = (
	product_id: string,
	product_url: string,
) => {
	const queryClient = useQueryClient();
...
export const useDeletePhotosGroup = (
	product_id: string,
	product_url: string,
) => {
	const queryClient = useQueryClient();
...
export const useDeletePhotoFromGroup = (
	product_id: string,
	product_url: string,
) => {
	const queryClient = useQueryClient();
